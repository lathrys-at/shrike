...
When a vector index is available (and not a dry run), each created or updated result includes `neighbors`: near-duplicate candidates from **two complementary signals**. The semantic match (cosine over the vector index, gated by `neighbor_threshold`) catches *paraphrase* dupes; a trigram lexical-overlap check over the derived-text store catches *near-verbatim* restatements that score marginally on embeddings — with no cosine to report, such a hit carries `score: null`. Every neighbor carries `provenance` (`[{signal, rank}]`, the same shape as search provenance): `text` for the semantic match, `fuzzy` for the lexical overlap, both when both fired. Semantically-scored candidates list first (descending score), lexical-only ones after. Use these for tag consistency (adopt tags from nearby notes), spotting near-duplicates, or understanding where a new note sits in the collection.
...
| `neighbor_threshold` | `number` | no | Minimum cosine similarity for a *semantically-ranked* neighbor. Default `0.6` — deliberately above the search default: dedup is a precision answer an agent acts on (a false dupe-flag suppresses a legitimately new card), and near-verbatim restatements below the gate are still caught by the lexical-overlap signal, which has no cosine gate. |
