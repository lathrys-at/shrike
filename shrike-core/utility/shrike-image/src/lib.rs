...
//! the CLIP vision path needs, reusable, independently testable, and free of
//! any engine/kernel coupling.
//!
//! The pipeline mirrors the Python CLIP `_preprocess` (PIL): decode
...
//! autovectorizes — a strided HWC→CHW scatter would instead do ~`3·crop²`
//! scalar divides per image (~150k at 224²). It is the *same* affine math as the
//! scalar reference, so it matches within the golden image-prep tolerance; off,
//! the transform degrades to a plain scalar loop (minimal-core). The resize
//! stays `image`'s scalar Catmull-Rom throughout.
//!
...
//! SIMD resize backend stays out: `fast_image_resize`'s Catmull-Rom differs
//! from `image`'s by ~0.078 on a real downscale, far past the tolerance, so it
//! is genuinely vector-affecting and belongs behind a consumer-fingerprint
//! mechanism, not this crate's default.)
