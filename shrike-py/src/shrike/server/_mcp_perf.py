...
Caching validator *compilation* fixes the small payloads — but the
data-proportional instance WALK survives, and at scale it dominates: measured
6.5 ms (output, 200-note response) + 2.5 ms (input, 100-note upsert) per call
against ~0.5 ms for everything else in the Python layer combined. Both ends are
enforced by Pydantic regardless (typed params in, response models out — with
better error messages), so the walk is skipped entirely. A malformed input still
fails the tool call: FastMCP's Pydantic parse rejects it; the SDK's jsonschema
pass was a second, slower spelling of the same check.
...
class _CachingJsonschema:
    ...
    Every other attribute (``ValidationError``, ``exceptions``, …) forwards to
    the real module.
    """
    ...
    def validate(self, instance: Any, schema: Any) -> None:
        # Deliberately a no-op: both ends of every Shrike tool call are already
        # Pydantic-validated; the SDK's per-call jsonschema walk is a redundant
        # O(payload) pass (~9 ms on the large calls).
        return None
