...
The index itself lives in the kernel: ``shrike-kernel``'s ``IndexOrchestrator``
(``shrike-core/runtime/shrike-kernel/src/index_orchestrator.rs``) owns the
per-modality USearch engine, drift detection, incremental reconcile, per-note
fingerprints, persistence, the debounced saver, and activation calibration. The
unit suites drive a real ``AsyncKernel`` (``tests/unit/conftest.py``).
...
- :func:`activation_floor` — the pure gate math the search action applies to
  the kernel-calibrated stats.
...
# Intra-modal activation gate. A non-text modality's ranking is fed to the fusion only when
