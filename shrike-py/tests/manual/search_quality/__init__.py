"""Shared search-quality support (#559): the metric engine + manifest loader.

One source of truth for what a search-quality run computes, imported by two
consumers:

- ``tests/native/test_search_quality.py`` (PR1) — the DETERMINISTIC,
...
- ``tests/integration/test_search_quality.py`` (PR2, manual/off-CI) — the
...
so it doubles as #234's threshold-tuning harness (re-run over
``threshold``/``ACTIVATION_MARGIN``/weights/``RRF_K``). The manifest loader
(:mod:`manifest`) parses the reconciled cards/queries schema both PRs share.
"""
