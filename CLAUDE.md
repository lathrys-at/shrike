...
├── tools.py                      # Registers 14 MCP tools; returns response models (emits outputSchema)
...
| `find_replace_in_note_type` | Working | Find/replace text in one note type's template HTML + CSS (front/back/css selectors); returns a count |
