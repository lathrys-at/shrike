...
```
CLI (shrike)  ──HTTP/JSON-RPC──▶  MCP Server (FastMCP, server/ = the host; api/ = the verb surface)
                                      │
                                      └──▶ Harness (harness/harness.py: assembly + operational verbs)
                                              │
...
├── errors.py                     # ShrikeError taxonomy (client-side; the wire-less error contract)
├── client.py                     # ShrikeClient — standalone HTTP client; typed per-tool methods
├── platform/                     # Near-leaf process infra (bottom of the layering)
...
├── harness/                      # Kernel-wrapping assembly + the engine bindings
...
│   ├── index.py                  # IndexState + activation_floor + ACTIVATION_MARGIN (host-side index policy)
│   ├── derived.py                # DerivedTextStore — FTS5 facade (read paths; kernel ingests in server mode)
│   ├── profiles.py               # Capability/build profiles (parse/resolve/plan onto the runtime)
│   ├── registry.py               # Collection/profile registry (#66) — name → collection-path
│   ├── cache_layout.py           # Per-collection namespaced index/derived cache paths (#68/#547)
│   ├── search_fusion.py          # rrf_fuse — the frozen Python reference for shrike_kernel::fusion
│   └── engines/                  # Engine bindings attached to the kernel's slots (mirror Rust engines/)
│       ├── embedding/            # The embed runtime + the in-process backends
│       │   ├── base.py           # EmbedderBackend protocol + NoteEmbedInput
│       │   ├── runtime.py        # EmbedderBackend facades + EmbeddingRuntime (backend lifecycle)
│       │   ├── onnx.py           # OnnxBackend (in-process onnxruntime + tokenizers, text)
│       │   ├── onnx_common.py    # Shared onnxruntime provider resolution
│       │   ├── clip.py           # ClipBackend (in-process CLIP dual-encoder, text+image)
│       │   ├── batching.py       # Batch-safety probe (probe_max_safe_batch)
│       │   └── text.py           # EMBED_TEXT_VERSION pin (normalize_for_embedding lives crate-side)
│       └── recognition/          # OCR today; room for ASR/describe facades (a package, #730)
│           ├── __init__.py       #   re-exports make_recognizer / make_describe_recognizer
│           └── recognition.py    # RecognizerBackend protocol + make_recognizer (OCR; native engine)
├── api/                          # The operation/verb surface
...
├── server/                      # The MCP host (a package, #730)
│   ├── __init__.py               # re-exports main; __main__.py preserves `python -m shrike.server`
│   ├── __main__.py               # `python -m shrike.server` entry point
│   ├── server.py                 # MCP host: argparse, FastMCP, routes; main() tail = asyncio.run(_serve())
│   ├── _mcp_perf.py              # Per-call jsonschema validator cache (#140)
...
is reunited there beside the sibling `tools/bazel.lock` build-pin; a script a
developer runs by hand stays in `scripts/` even when its output feeds a build.
